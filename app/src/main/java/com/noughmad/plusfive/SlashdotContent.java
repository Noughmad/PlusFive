...
public class SlashdotContent {
	...
	public static boolean refreshStories(Context context, String source) {
		...
		for (Element article : articles) {
            ...
            try {
                values.put(SlashdotProvider.STORY_SUMMARY, article.select("div#text-" + id + " i").first().html());
                values.put(SlashdotProvider.STORY_COMMENT_COUNT, Integer.parseInt(article.select("span.comment-bubble a").first().html()));
	...
	private static void parseComment(Context context, Uri baseUri, Element tree, int level, String parentTitle) {
		...
		if (!tree.hasClass("hidden")) {
            ...
            } else if (!comment.select("span.by").isEmpty()){
